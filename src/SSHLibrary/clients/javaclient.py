...
class JavaSSHClient(AbstractSSHClient):
    ...
    def _login(
        ...
        password,
        allow_agent="ignored",
        look_for_keys="ignored",
        proxy_cmd=None,
        jumphost_alias_or_index=None,
        read_config=False,
        keep_alive_interval=None,
    ):
        if allow_agent or look_for_keys or keep_alive_interval:
            raise JavaSSHClientException(
                "Arguments 'allow_agent', 'look_for_keys', "
                "`jumphost_index_or_alias` and `keep_alive_interval`"
                " do not work with Jython."
            )

        auth = (
            self.client.authenticateWithPassword(username, password)
            if password
    ...
    def _login_with_public_key(
        ...
        try:
            success = self.client.authenticateWithPublicKey(
                username, File(key_file), password
            )
            if not success:
                raise SSHClientException
        except IOError:
    ...
    def create_local_ssh_tunnel(self, local_port, remote_host, remote_port, *args):
        self.client.createLocalPortForwarder(
            int(local_port), remote_host, int(remote_port)
        )
        logger.info(
            "Now forwarding port %s to %s:%s ..."
...
class SFTPClient(AbstractSFTPClient):
    ...
    def _get_file(self, remote_path, local_path, scp_preserve_times):
        ...
        offset = 0
        while True:
            read_bytes = self._client.read(
                remote_file, offset, data, 0, array_size_bytes
            )
...
class SCPClient(object):
    ...
    def put_directory(self, source, destination, *args):
        raise JavaSSHClientException(
            "`Put Directory` not available with `scp=ALL` option. Try again with "
            "`scp=TRANSFER` or `scp=OFF`."
        )

    def get_directory(self, source, destination, *args):
        raise JavaSSHClientException(
            "`Get Directory` not available with `scp=ALL` option. Try again with "
            "`scp=TRANSFER` or `scp=OFF`."
        )
...
class SCPTransferClient(SFTPClient):
    ...
    def _put_file(
        self, source, destination, mode, newline, path_separator, scp_preserve_times
    ):
